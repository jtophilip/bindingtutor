function [Abound, Afree] = dimer_saturation(MTtot, Atot, K1, K2, KA, N)
% A function which calculates the binding of A to MT assuming A can bind to
% MT either as a monomer, with a KD of K1, or as a dimer with a KD of K2.
...
for n = 1:b
    ...
    f = @(A)A + 2*A^2/KA + (A/K1 + 2*A^2/(KA* K2))*MTtot*N/(1 + A/K1 + 2*A^2/(KA*K2)) - Atot(n);
