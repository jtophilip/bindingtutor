function [Frac, MTfree] = pseudocooperativity_binding(MTtot, Atot, KAM, KAMP, N)
...
for n = 1:b
    ...
    f = @(A)A + ((A/KAM) + A^2/(KAM*KAMP))*MTtot(n)/(1 + 2*A/(KAM*N) + A^2/(KAM*KAMP*N)) - Atot;
...
% Solves for free MT
MTfree = (1 + Afree./(KAM*N)).*MTtot./(1 + 2*Afree./(KAM*N) + Afree.^2./(KAM*KAMP*N));
