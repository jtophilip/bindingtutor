function [Frac] = sites_binding(MTtot, Atot, KA1, KA2)
% A function which calculates the binding of A to MT assuming that A binds
% to two indepenent sites on the MT with dissociation constants KA1 and KA2 for an experiment where [MT] is
...
% Sets the vectors for total MT1 and MT2
MT1 = MTtot;
MT2 = MTtot;
...
for n = 1:b
    ...
    f = @(A)A + (1/KA1)*A*MT1(n)/(1 + (1/(KA1))*A) + (1/KA2)*A*MT2(n)/(1 + (1/(KA2))*A) - Atot;
