function [Frac, MTfree] = MAP_binding(MTtot, Atot, KAM, KAA, N)
% A function which calculates the binding of A to MT assuming that A binds
% to MT with a disassociation constant of KAM and that a second A can bind
% to an MT-bound A with a disassociation constant of KAA for an experiment
...
for n = 1:b
    ...
    f = @(A)A + (A/KAM + 2*A^2/(KAA*KAM))*MTtot(n)/(1 + A/(KAM*N) + A^2/(KAM*KAA*N)) - Atot;
...
% Calculates free MT
MTfree = MTtot./(1 + Afree./(KAM*N) + Afree.^2./(KAM*KAA*N));
